...
use IsmaAmbrosi\Bundle\GeneratorBundle\Generator\DoctrineApiGenerator;

class GenerateDoctrineApiCommand extends GenerateDoctrineCrudCommand
{
    /**
     * @var DoctrineApiGenerator
     */
    private $generator;
    ...
    protected function configure()
    {
        $this
            ...
            ->setDescription('Generates a API based on a Doctrine document')
            ->setHelp(<<<EOT
The <info>doctrine:mongodb:generate:api</info> command generates an API based on a Doctrine document.

The default command only generates the list and show actions.

<info>php app/console doctrine:mongodb:generate:api --document=AcmeBlogBundle:Post --route-prefix=post_admin</info>

Using the --with-write option allows to generate the new, edit and delete actions.

<info>php app/console doctrine:mongodb:generate:api --document=AcmeBlogBundle:Post --route-prefix=post_admin --with-write</info>
EOT
            )
            ->setName('doctrine:mongodb:generate:api')
            ->setAliases(['generate:doctrine:mongodb:api'])
        ;
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $baseName = $input->getOption('route-basename');
        $withWrite = $input->getOption('with-write');

        $dialog->writeSection($output, 'API generation');
        ...
        $generator->generate($bundle, $document, $metadata, $format, $prefix, $baseName, $withWrite);
    ...
    protected function interact(InputInterface $input, OutputInterface $output)
    {
        ...
        $this->askForRouteName($input, $output, $dialog, $document);
    ...
    protected function getGenerator()
    {
        if (null === $this->generator) {
            $this->generator = new DoctrineApiGenerator($this->getFilesystem(), $this->getSkeletonPath().'/api');
        }
    ...
    private function updateRouting(QuestionHelper $dialog, InputInterface $input, OutputInterface $output, BundleInterface $bundle, $format, $document, $prefix)
    {
        ...
        $configPath = $bundle->getPath().'/Resources/config/api';
        ...
        $help = sprintf("        <comment>resource: \"@%s/Resources/config/api/routing/%s.%s\"</comment>\n", $bundle->getName(), strtolower(str_replace('\\', '_', $document)), $format);
     ...
     * @param                 $document
     */
    private function askForRouteName(InputInterface $input, OutputInterface $output, QuestionHelper $dialog, $document)
    {
        $basename = $this->getRoutePrefix($input, $document) . '_api';
        $output->writeln(array(
            '',
            'Determine the base name of the routes (e.g. user_api).',
            '',
        ));
        $question = new Question($dialog->getQuestion('Routes base name', $basename), $basename);
        $basename = $dialog->ask($input, $output, $question, $basename);
        $input->setOption('route-basename', $basename);
    }
}
